...
def google_maps_config():
    values = {
        ...
        "map_id": os.environ.get("NEXT_PUBLIC_GOOGLE_MAPS_MAP_ID"),
    }
    ...
    if env_path.exists():
        for line in env_path.read_text(encoding="utf-8").splitlines():
            ...
            if key == "NEXT_PUBLIC_GOOGLE_MAPS_API_KEY" and not values["api_key"]:
                values["api_key"] = value.strip()
            if key == "NEXT_PUBLIC_GOOGLE_MAPS_MAP_ID" and not values["map_id"]:
                values["map_id"] = value.strip()
    return values
...
ELEVATION_NC_PATH = str(BASE_DIR / "data/gebco_2026_sub_ice_n34.954_s2.877_w63.003_e90.804.nc")
POPULATION_TIF_PATH = str(BASE_DIR / "data/ind_pop_2026_CN_100m_R2025A_v1.tif")
...
def plot_interactive_map(damage_grid, lats, lons, land_mask, storm_lat, storm_lon, out_path):
    ...
    for i, latitude in enumerate(lats):
        for j, longitude in enumerate(lons):
            value = damage_grid[i, j]
            if land_mask[i, j] and not np.isnan(value) and value > 0.05:
                heat_data.append([float(latitude), float(longitude), float(value)])

    maps_config = google_maps_config()
    api_key = maps_config["api_key"] or "YOUR_GOOGLE_MAPS_API_KEY"
    map_id = maps_config["map_id"] or ""
    map_id_option = f'mapId: "{map_id}",' if map_id else ""
    output_path = Path(out_path)
    output_path.write_text(f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0" />
<title>Cyclone damage risk</title>
<style>html,body,#map{{width:100%;height:100%;margin:0}}body{{background:#0f1b2b}}</style>
</head><body><div id="map"></div><script>
const heatData = {json.dumps(heat_data)};
function initMap() {{
  const center = {{lat:{float(storm_lat)},lng:{float(storm_lon)}}};
    const map = new google.maps.Map(document.getElementById("map"), {{center,zoom:6,{map_id_option}}});
  new google.maps.visualization.HeatmapLayer({{data:heatData.map(([lat,lng,weight]) => ({{location:new google.maps.LatLng(lat,lng),weight}})), radius:18, opacity:.78}}).setMap(map);
  new google.maps.Marker({{position:center,map,title:"Storm center"}});
}}
</script><script async src="https://maps.googleapis.com/maps/api/js?key={api_key}&libraries=visualization&callback=initMap"></script>
</body></html>""", encoding="utf-8")
    print(f"Saved Google Maps HTML to {output_path}")


def export_ground_overlay(damage_grid: np.ndarray, output_path: Path, min_alpha=35, max_alpha=215):
    """Write the damage grid as a transparent, north-up PNG overlay."""
    from PIL import Image

    cmap = mcolors.LinearSegmentedColormap.from_list(
        "risk_overlay", ["#2ecc71", "#f1c40f", "#e67e22", "#e74c3c", "#8e0000"]
    )
    ...
    rgba[..., 3] = np.where(
        valid,
        min_alpha + clipped * (max_alpha - min_alpha),
        0,
    ).astype(np.uint8)
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    Image.fromarray(np.flipud(rgba), mode="RGBA").save(output_path)


def export_web_bundle(damage_grid, lat_min, lat_max, lon_min, lon_max, storm, output_dir):
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    export_ground_overlay(damage_grid, output_dir / "overlay.png")
    manifest = {
        "bounds": {
            "south": float(lat_min),
            "west": float(lon_min),
    ...
    (output_dir / "overlay.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    return manifest
...
def main(
    ...
    if output_dir is not None:
        manifest = export_web_bundle(
            damage_grid, LAT_MIN, LAT_MAX, LON_MIN, LON_MAX, STORM, output_dir
        )
